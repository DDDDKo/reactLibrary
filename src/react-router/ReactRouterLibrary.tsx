...
// - 웹 애플리케이션 내에서 URL 이동이 있을때 웹 페이지를 새로고침 하지 않도록 함
// - 웹 애플리케이션 내에서 네비게이션 역할을 수행

// npm i react-router react-router-dom

// - 루트 경로에 있는 index.tsx의 root.render() 안에 포함되어 있는 <App /> 컴포넌트를 (=>root.render(<App />)) <BrowserRouter> 컴포넌트로 묶어야함
// - <BrowerRouter> : URL을 사용하여 브라우저 주소 표시줄에 현재 위치를 저장하고 탐색할 수 있도록 하는 컴포넌트

// - 특정 URL 패턴에 대해서 컴포넌트를 각각 다르게 랜더링하고자 한다면 App.tsx에 <Routes> 컴포넌트와 <Route> 컴포넌트를 이용하여 경로에 따른 컴포넌트를 지정

// <Routes> :
// - <Route> 컴포넌트로 URL에 따른 컴포넌트를 랜더링 할 수 있도록 하는 컴포넌트

// <Route> :
// - URL 패턴에 따라서 랜더링하고자 하는 컴포넌트를 지정하는 컴포넌트
// - path 속성 : URL 패턴 지정
// - element 속성 : 랜더링할 컴포넌트 지정
// - index 속성 : 현재 패턴에 기본 라우터로 지정

// http://localhost:3000
// http://localhost:3000/router
export default function ReactRouterLibrary() {

    // - a 요소로 페이지 전환을 하게 되면 새로운 요청을 보내게 되어 반드시 페이지가 새로고침 됨
    // - 페이지가 새로 고쳐지면 상태 유지가 안 됨
    // 이를 해결하기위해 Link 컴포넌트 사용

    // <Link> :
    // - 사용자가 해당 웹 애플리케이션 내에서 다른 페이지로 이동할 수 있도록 하는 컴포넌트
    // - a 요소의 href로 페이지 전환을 하게 되면 새로운 페이지로 전향되는 반면 <Link> 컴포넌트는 새로고침을 하지 않고 URL만 변경
    ...
    const onButtonClick = () => {
        if(count >= 10) return navigator('/zustand');
        setCount(count + 1);
    };
    ...
    <div>
        ...
        {/* <a href="http://localhost:3000/router/page1">page1!</a> */}
        <Link to={'/router/page1'}>page1</Link>
        {/* <a href="http://localhost:3000/router/page2">page2!</a> */}
...
export function ReactRouterPage1 () {
    ...
    // - 해당 URL 경로에 대한 모든 동적 URL 패턴을 name, vlaue 형태로 받아옴
...
export function ReactRouterPage2 () {
    ...
    // useSearchParams :
    // - 현재 URL에 있는 쿼리 문자열을 읽을 때 사용하는 함수
    // - 쿼리 문자열에 대한 배열을 반환

    // 쿼리 문자열 => url뒤에 ?뒤에 오는 값들 보통 key=value로 이루어져 있음 
    // useSearchParams를 사용하면 ?뒤에 오는 값만 읽음
    // path를 읽을때는 ?뒤에 오는 쿼리 문자열은 읽지 않음

...
// - react에서 cookie를 쉽게 관리할 수 있도록 도움을 주는 라이브러리
// - cookie 설정, 검색, 삭제를 간단한 함수로 작업할 수 있도록 함
...
export default function ReactCookieLibrary() {
    ...
    const onValueChangeHandler = (evnet: ChangeEvent<HTMLInputElement>) => {
        setValue(evnet.target.value);
    };

    const onButtonClickHandler = () => {
        // cookies 객체는 브라우저가 가지고 있는 cookie 정보를 객체로 가져옴
        console.log(cookies);
    };
    ...
    const onRegistButtonClickHandler = () => {
        ...
        // setCookie 함수는 첫번째 매개변수로 name 값 / 두번째 매개변수로 value 값 / 세번째 매개변수로 option을 지정
        // option : 
        // - path : 쿠키를 사용할 수 있는 경로 지정
        // - expires : 쿠키 만료 시간 지정
        setCookie(key, value, { path: '/', expires: now });
    };

    const onRemoveButtonClickHandler = () => {
        // removeCookie 함수는 첫번째 매개변수로 name값 / 두번째 매개변수로 option을 지정
        // - option은 set의 option과 동일
    ...
    return (
        <div>
            ...
            <button onClick={onButtonClickHandler}>쿠기 확인</button>

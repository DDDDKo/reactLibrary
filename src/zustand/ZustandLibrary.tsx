...
// - Redux, MobX 와 같은 타 상태관리 라이브러리들에 비해 가벼움 => 빌드 할때 패키징 속도 향상/ 빌드 후 파일의 크기(번들의 크기)가 작아짐

// zustand를 이용한 글로벌 상태 선언 방법
// 1. zustand의 create 함수를 사용하여 store를 생성
// store : 상태와  상태 관리 로직을 하나로 묶은 객체
// store 생성하는 create 함수의 반환 데이터는 훅 함수를 반환 (그렇기 때문에 use라는 이름의 함수명으로 받아야함)

// - create 함수의 매개변수로 set 인자를 받는 콜백 함수를 전달해야함
// - 매개변수로 전달한 콜백함수는 store 객체를 반환해야함
// - store 객체는 상태를 가지고있음, store 객체를 변경하는 함수가 포함됨  store=> 상태 + 변경함수

// - typescript에서 zustand의 create함수를 사용할때는 create 함수의 제너릭으로 store(상태)의 타입을 지정해야함
...
// create 함수의 콜백 함수가 받는 set인자는 상태 변경을 위한 함수
const useStore = create<Store>((set) => ({
    zNormal: 0,
    // set 함수는 매개변수로 현자 상태(state)를 인자로 받는 콜백 함수를 전달해야함
